...
export const Profile = () => {
  ...
  const handleUserPhotoSelected = async() =>{
    ...
    try {
      ...
      if(photoSelected.assets[0].uri) {
        ...
        const photoFile = {
          name: `${user.email}.${fileExtension}`.toLowerCase(),
        ...
        const avatarUpdtedResponse = await api.patch('/v1/user/avatar', userPhotoUploadForm, {
          headers: {
  ...
  const handleProfileUpdate = async (data: FormDataProps) => {
    try {
      ...
      await api.put('/v1/user/update', data);
